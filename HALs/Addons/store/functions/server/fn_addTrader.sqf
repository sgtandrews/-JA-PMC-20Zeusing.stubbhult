...
try {
...

	{
		...
		_items = missionNamespace getVariable _x select 1;
		diag_log "_items";
		diag_log _items;
		};
		...
		private _stockAmount = nil;
		if (isNumber (missionConfigFile >> "CfgHALsAddons" >> "cfgHALsStore" >> "categories" >> _x >> "defaultstock")) then {_stockAmount = getNumber(missionConfigFile >> "CfgHALsAddons" >> "cfgHALsStore" >> "categories" >> _x >> "defaultstock")} else {_stockAmount = (missionNamespace getVariable _category select 2)};
		{
			_classes pushBack _x;
			_stocks pushBack toLower _x;
			if (isNil {_stockAmount}) then 
			{
				if (isClass (_configCategory >> _x)) then {
					_stocks pushBack (getNumber (_configCategory >> _x >> "stock") max 0);
					//_stocks pushBack (missionNamespace getVariable _category select 2);
				} else {
					_stocks pushBack (missionNamespace getVariable _category select 2);
				};
			...
			} else 
			{
				...
				_stocks pushBack _stockAmount;
			};
		} forEach _items;
	if (isDedicated) then {sleep 0.1};
	diag_log str(_traderType);
	diag_log str(_x);
	diag_log str(_stocks);
	} forEach _categories;

...
if(_sel != (localize "STR_HG_NONE")) then
{
	...
	[
		...
		(side HG_OBJECT),
		(toUpper(getText(getMissionConfig "CfgClient" >> "HG_MasterCfg" >> (rank HG_OBJECT) >> "displayName"))),
		_xpText,
	...
	
	{
		...
		{
		    _ind = HG_ADM_COMBO lbAdd (toUpper(getText(getMissionConfig "CfgClient" >> "HG_MasterCfg" >> _x >> "displayName")));

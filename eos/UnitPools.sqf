...
// EAST BANDITS
	if (_faction==5) then {
	_InfPool=	["CUP_O_RUS_M_Soldier_Survivor_VKPO_Summer"];
	_ArmPool=	["rhsgref_ins_uaz_dshkm","CFP_O_SOREBEL_Technical_PK_01"];
	_MotPool=	["rhsgref_ins_uaz_open","rhsgref_ins_uaz_dshkm","rhsgref_ins_uaz","CFP_O_SOREBEL_Technical_PK_01","CFP_O_SOREBEL_Truck_01"];
	_ACHPool=	[];
	_CHPool=	[];
	_uavPool=	[];
	_stPool=	[];
	_shipPool=	[];
	_diverPool=	[];
	_crewPool=	["CUP_O_RUS_M_Soldier_Survivor_VKPO_Summer"];
	_heliCrew=	[];
	};

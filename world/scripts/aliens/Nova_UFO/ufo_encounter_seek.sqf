...
_poz = hunt_alias getRelPos [100 + (((call NOVA_FNC_RAND) mod 500) + 1), selectrandom [(((call NOVA_FNC_RAND) mod 60) + 1), (((call NOVA_FNC_RAND) mod 60) * -1 - 1)]];
"seekermarker" setMarkerPos _poz;

seek_encounter_running = false;

NOVA_FNC_seek_travel_fx = {
  params ["_ufo"];

  if (!seek_encounter_running) then {
    seek_encounter_running = true;
    enableCamShake true;
    ...
    _blast_wave = "#particlesource" createVehicle getPosATL _orb_reper;
    ...
    _orb_obj = "#lightpoint" createVehicle getmarkerpos _ufo;
    ...
    while {_jumps>0} do {
    	...
    	while {_rand_arc>0} do {
    		...
    		_orb_obj setpos [_poz_rel_1#0,_poz_rel_1#1,2];//was 2
